...
import { exportarClientesCsv, exportarManutencaoCsv, exportarDespachosCsv, exportarHistoricoManobrasCsv, exportarHistoricoAbastecimentoCsv } from '../lib/exportarPlanilha'
import { buscarMarina, atualizarConfigMarina, listarCombustiveis, salvarCombustivel } from '../lib/db'
import { textoQuantidade, momentoConfirmacaoAbastecimento } from '../lib/statusAbastecimento'
...
export default function ConfiguracoesPainel({
  ...
  // Histórico de abastecimento — pedidos de combustível já confirmados
  // (pela equipe ou pelo relógio dos 15 minutos), que saíram da planilha de
  // trabalho do Painel de Controle e vivem só aqui a partir de agora (ver
  // pedidosCombustivel/historicoAbastecimento em TelaVagas.jsx).
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              ...
              <table className="tabela">
                ...
                <tbody>
                  {historicoAbastecimento.length === 0 && <tr><td colSpan={5}>Nenhum abastecimento confirmado ainda.</td></tr>}
                  {historicoAbastecimento.map((p) => (
                    <tr key={p.id}>
                      <td className="col-responsavel"><b>{p.clientes?.nome}</b>{p.embarcacoes?.nome ? ` · ${p.embarcacoes.nome}` : ''}</td>
                      <td>{p.combustiveis?.nome || '—'}</td>
                      <td>{textoQuantidade(p)}</td>
                      <td>{new Date(p.created_at).toLocaleString('pt-BR')}</td>
                      <td>{(() => { const m = momentoConfirmacaoAbastecimento(p); return m ? new Date(m).toLocaleString('pt-BR') : '—' })()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
