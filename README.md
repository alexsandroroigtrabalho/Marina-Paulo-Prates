...
- **Painel de Controle** *(diferencial competitivo — pensado para ficar aberto numa smart TV na marina, e é a tela inicial da área interna)* — relógio ao vivo e a **Fila de Rampa**: uma tabela em tempo real com cada notificação de descida ou subida, atualizada sozinha a cada 45s. Tela em `TelaVagas.jsx`. *Esta marina não trabalha com reserva de vaga/pier — só com a fila de retirada e retorno.*
  - Os contadores (embarcações na água / serviços em aberto / abastecimentos pendentes) ficam como pílulas ao lado do nome da marina, no topo — em vez de ocuparem uma linha inteira própria, o que deixa mais altura de tela pras tabelas.
  - Cada linha tem um indicativo luminoso: 🟢 verde para pedido de descida, 🔴 vermelho para pedido de subida. A coluna **Documentação** mostra **Regular** ou **Pendente**, a partir dos documentos cadastrados da embarcação (TIE, seguro, habilitação, vistoria) — pendente se algo estiver vencido ou nada tiver sido cadastrado ainda.
  - Cada notificação tem só um clique de ação ("Confirmar saída" / "Confirmar retorno") — sem etapas intermediárias de "confirmado"/"em andamento". Ao confirmar a saída, a notificação sai sozinha da Fila de Rampa e passa a aparecer na tabela **Navegando** logo abaixo — só com o essencial (embarcação, responsável, horário de saída, previsão de retorno e status). Some de lá quando o retorno daquela embarcação for confirmado. Pedidos de serviço (despachos/laudos) não têm mais uma lista própria no meio do painel — ficam só na pílula "Serviços em aberto" no topo.
  ...
  - As ações do painel (ativar sons, histórico de manobras, gerenciar combustíveis) ficam num **menu de engrenagem** ⚙️ no cabeçalho, do lado do nome do usuário logado — em vez de botões fixos em cima da Fila de Rampa, o que deixa a tela mais limpa.
  - **"Histórico de manobras"** abre o registro permanente de toda descida e subida já confirmada (mais recente primeiro) — diferente da tabela Navegando, o registro fica ali mesmo depois que a embarcação retorna. **"Gerenciar combustíveis"** abre o cadastro de combustíveis (nome, preço por litro, estoque).
  - **Sinal sonoro**: assim que uma notificação nova entra na Fila de Rampa, o painel toca sozinho um apito — **um apito longo para pedido de descida** e **três apitos curtos para pedido de retorno** —, útil pra equipe perceber sem precisar ficar olhando pra TV o tempo todo. Por padrão o som é sintetizado na hora (Web Audio API — fundamental grave + harmônicos + uma camada de ruído filtrado imitando o ar comprimido de uma buzina pneumática real), sem depender de nenhum arquivo de áudio externo. Se você quiser usar uma **gravação real** de buzina de navio em vez do som sintetizado, basta colocar `buzina-longa.mp3` e `buzina-curta.mp3` dentro de `public/sons/` (detalhes e sugestões de onde baixar em `public/sons/LEIA-ME.txt`) — o painel detecta e passa a usar os arquivos automaticamente, sem precisar mexer em código. Como o navegador só libera áudio depois de uma interação da pessoa na página, **"Ativar sons"** (no menu de engrenagem) precisa ser clicado uma vez ao abrir a TV/tela — depois disso fica ativo enquanto a página estiver aberta. Só toca para notificações que chegam *depois* da ativação — o que já estava esperando quando o painel foi aberto não dispara som.
