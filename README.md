...
- **Painel de Controle** *(diferencial competitivo — pensado para ficar aberto numa smart TV na marina, e é a tela inicial da área interna)* — relógio ao vivo, contadores de embarcações na água / pedidos de serviço em aberto / abastecimentos pendentes, e a **Fila de Rampa**: uma tabela em tempo real com cada notificação de descida ou subida, atualizada sozinha a cada 45s. Tela em `TelaVagas.jsx`. *Esta marina não trabalha com reserva de vaga/pier — só com a fila de retirada e retorno.*
  ...
  - Só existem 3 status por notificação: **Aguardando descida**, **Aguardando retorno** ou **Navegando**. Sem etapas intermediárias de "confirmado"/"em andamento" — o operador dá um clique ("Confirmar saída" / "Confirmar retorno") e o status muda na hora; uma notificação de descida confirmada vira automaticamente **Navegando** e sai sozinha da Fila de Rampa, passando a aparecer na tabela **Navegando** logo abaixo — só com o essencial (embarcação, responsável, horário e abastecimento vinculado, sem repetir tipo de pedido, indicativo luminoso ou status) — e some de lá quando o retorno daquela embarcação for confirmado. Pedidos de serviço (despachos/laudos) não têm mais uma lista própria no meio do painel — ficam só no contador "Pedidos de serviço em aberto" no topo.
